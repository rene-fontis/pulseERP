...
const accountSchema = z.object({
  ...
  balance: z.number().optional().default(0), // Balance is typically not edited here but part of the data model
});
      ...
      ? { 
          ...
          groups: initialData.groups.map(g => ({
            ...
            accounts: g.accounts.map(a => ({
                ...
                balance: a.balance || 0, // Ensure balance
            })) 
          }))
        } 
      : { name: '', groups: [{ id: crypto.randomUUID(), name: 'Gruppe 1', mainType: 'Asset', accounts: [{id: crypto.randomUUID(), number: '1000', name: 'Kasse', description: '', balance: 0}] }] },
  });
    ...
    <div className="space-y-3 pl-4 border-l border-border">
      ...
      {fields.map((accountItem, accountIndex) => (
        <Card key={accountItem.id} className="p-3 bg-muted/30 relative">
          ...
          />
          {/* Balance field is part of the model but not typically edited here. 
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="mt-2">
                <FormLabel className="text-xs">Saldo (CHF)</FormLabel>
                <FormControl>
                  <Input type="number" {...field} readOnly className="h-8 text-sm bg-muted/50" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
      ...
      <Button
        ...
        variant="outline"
        size="sm"
        onClick={() => append({ id: crypto.randomUUID(), number: '', name: '', description: '', balance: 0 })}
